...
class PhysicalCpuCores(UsageBasePlugin):
    def usages(self, **kwargs):
        ...
        :rtype dict:
        """
        logger.debug("Get physical cpu cores usage")
        usage_type = UsageType.objects.get(name="Physical CPU cores")
        core_usages = self.get_usages_and_costs(
            kwargs['start'],
            kwargs['end'],
            kwargs['ventures'],
            usage_type,
