...
class BackOfficeAssetInfo(PricingObject):
    ...
    def get_daily_pricing_object(self, date):
        return self.daily_back_office_asset_info_set.get_or_create(
            date=date,
            defaults=dict(
                pricing_object=self,
                back_office_asset_info=self,
                service_environment=self.service_environment,
            )
        )[0]
...
class DailyBackOfficeAssetInfo(DailyPricingObject):
    back_office_asset_info = db.ForeignKey(
        BackOfficeAssetInfo,
        related_name="daily_back_office_asset_info_set",
