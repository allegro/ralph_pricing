...
from ralph_scrooge.management.commands import pricing_sync


COLLECT_PLUGINS = ['asset', 'business_line', 'warehouse']
COMMAND_NAME = pricing_sync.Command.__module__.split('.')[-1]


class TestPricingSync(TestCase):
    def test_load_plugins(self):
        pricing_sync._load_plugins()
        self.assertTrue(pricing_sync.PLUGINS_LOADED)
    ...
    def test_get_collect_plugins_names(self):
        self.assertEquals(
            pricing_sync.get_collect_plugins_names(),
            COLLECT_PLUGINS
        )

    @patch('ralph_scrooge.management.commands.pricing_sync.plugin.run')
    def test_run_plugin(self, plugin_run_mock):
        plugin_run_mock.return_value = True, 'Everything OK'
        today = datetime.date.today()
        pricing_sync._run_plugin('abc', today)
    ...
    def test_run_plugin_failure(self, plugin_run_mock):
        ...
        today = datetime.date.today()
        with self.assertRaises(pricing_sync.PluginError):
            pricing_sync._run_plugin('abc', today)
        plugin_run_mock.assert_called_with('scrooge', 'abc', today=today)
    ...
    @override_settings(COLLECT_PLUGINS=COLLECT_PLUGINS)
    @patch('ralph_scrooge.management.commands.pricing_sync._run_plugin')
    def test_run_plugins(self, run_plugin_mock):
        def side_effect(name, today):
            if name == 'business_line':
                raise pricing_sync.PluginError()
            else:
                return None
        run_plugin_mock.side_effect = side_effect
        today = datetime.date.today()
        result = [r for r in pricing_sync.run_plugins(
            today,
    ...
    @override_settings(COLLECT_PLUGINS=COLLECT_PLUGINS)
    @patch('ralph_scrooge.management.commands.pricing_sync.run_plugins')
