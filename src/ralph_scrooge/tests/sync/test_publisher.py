...
from ralph_scrooge.tests.utils.factory import DailyCostFactory, EnvironmentFactory, ServiceEnvironmentFactory, \
    ServiceFactory, UsageTypeFactory
...
class TestPublisher(ScroogeTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.service_env_1 = ServiceEnvironmentFactory(service__ci_uid='uid-1', environment=cls.env_prod)
        ...
        ServiceEnvironmentFactory(service__ci_uid='uid-unused', environment=cls.env_test)

        usage_type = UsageTypeFactory()
        for day in range(1, 11):
            for i, se in enumerate([cls.service_env_1, cls.service_env_2, cls.service_env_3], start=1):
                DailyCostFactory(service_environment=se, cost=i*10, date='2018-09-{}'.format(day), forecast=True, type=usage_type)
                DailyCostFactory(service_environment=se, cost=i*10, date='2018-09-{}'.format(day), forecast=False, type=usage_type)

    @override_settings(
        ACCEPTED_COSTS_SYNC_RECIPIENTS=[(ANOTHER_SCROOGE_URL, ANOTHER_SCROOGE_TOKEN)],
    ...
    def test_should_send_dump_of_costs_to_recipient(self):
        ...
        self.assertEqual(request.headers['Authorization'], 'Token {}'.format(ANOTHER_SCROOGE_TOKEN))
