...
class TestProfitCenterPlugin(TestCase):
    def setUp(self):
        ...
        self.business_line1 = BusinessLineFactory(
            ci_uid=SAMPLE_PROFIT_CENTERS[0]['business_line']
        )
        self.business_line2 = BusinessLineFactory(
            ci_uid=SAMPLE_PROFIT_CENTERS[1]['business_line']
        )

    def _compare_profit_center(self, profit_center, sample_data):
        self.assertEquals(profit_center.name, sample_data['name'])
        self.assertEquals(profit_center.ci_uid, sample_data['ci_uid'])
        self.assertEquals(
            profit_center.description,
            sample_data['description']
        )
        self.assertEquals(
            profit_center.business_line.ci_uid,
            sample_data['business_line']
        )

    def test_add_profit_center(self):
        sample_data = SAMPLE_PROFIT_CENTERS[0]
        self.assertTrue(update_profit_center(sample_data, self.today))
        profit_center = ProfitCenter.objects.get(ci_uid=sample_data['ci_uid'])
    ...
    def test_update_profit_center(self):
        ...
        self.assertFalse(update_profit_center(sample_data2, self.today))
        profit_center = ProfitCenter.objects.get(ci_uid=sample_data2['ci_uid'])
    ...
    def test_batch_update(
        self, get_profit_centers_mock,
        update_profit_center_mock
    ):
        def sample_update_profit_center(data, date):
