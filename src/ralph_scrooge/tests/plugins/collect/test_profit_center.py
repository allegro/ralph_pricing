...
class TestProfitCenterPlugin(TestCase):
    def setUp(self):
        ...
        self.business_line1 = BusinessLineFactory(
            ci_id=SAMPLE_PROFIT_CENTERS[0]['business_segment']['id']
        )
        self.business_line2 = BusinessLineFactory(
            ci_id=SAMPLE_PROFIT_CENTERS[1]['business_segment']['id']
        )

    def _compare_profit_center(self, profit_center, sample_data):
        self.assertEquals(profit_center.name, sample_data['name'])
        self.assertEquals(profit_center.ci_id, sample_data['id'])
        self.assertEquals(
    ...
    def test_add_profit_center(self):
        ...
        profit_center = ProfitCenter.objects.get(ci_id=sample_data['id'])
    ...
    def test_update_profit_center(self):
        ...
        profit_center = ProfitCenter.objects.get(ci_id=sample_data2['id'])
    ...
    @mock.patch('ralph_scrooge.plugins.collect.profit_center.get_from_ralph')
