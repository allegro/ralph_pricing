...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ServiceOwnership = orm['ralph_scrooge.ServiceOwnership']
        TeamManager = orm['ralph_scrooge.TeamManager']
        Service = orm['ralph_scrooge.Service']

        for sos in ServiceOwnership.objects.all().select_related('owner', 'owner__user'):  # noqa
            sos.new_owner = sos.owner.user
            sos.save()

        for tm in TeamManager.objects.all().select_related('manager', 'manager__user'):  # noqa
            tm.new_manager = tn.manager.user
            tm.save()
