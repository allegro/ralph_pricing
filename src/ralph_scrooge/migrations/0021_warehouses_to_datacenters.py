...
# intentional use because these ones hasn't been visible in `orm` object
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Change source of Scrooge warehouses from assets Warehouses to assets
        datacenters.

        1. Remove Scrooge warehouses which are not:
            - default warehouse or
            - warehouse's name is not included in Ralph-asset's datacenter

        2. Update on left warehouses their id_from_assets to reflect ids
            from asset's Datacenter.
        """
        def ask_if_migrate():
            ...
            msg = WARN_MSG.format(
                ...
                left_count=undeleted.count(),
                left_names=left_names,
            )
            answer = raw_input(msg.encode('utf8'))
            return True if answer == 'yes' else False

        # keep only default warehouse or those whose name is in asset datacenter
        default_warehouse = orm['ralph_scrooge.warehouse'].objects.get(
            name__exact='Default',
        )
        keep_undeleted = [default_warehouse.name]
        dc_names = DataCenter.objects.values_list('name', flat=True)
        keep_undeleted.extend(dc_names[:])
        to_delete = orm['ralph_scrooge.warehouse'].objects.exclude(
            name__in=keep_undeleted,
        )

        # update id_from_asset from asset warehouse to asset datacenter
        datacenters = {
            name: obj_id
            for name, obj_id in DataCenter.objects.values_list('name', 'id')
        }

        migrate = ask_if_migrate()
        if not migrate:
            raise RuntimeError("Migration STOPPED by user")

        to_delete.delete()
        for warehouse in orm['ralph_scrooge.warehouse'].objects.filter(
            name__in=dc_names,
        ):
            datacenter_id = datacenters[warehouse.name]
            warehouse.id_from_assets = datacenter_id
            warehouse.save()
