...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        for fk_name in (
            ...
            try:
                db.delete_foreign_key('ralph_scrooge_dailycost', fk_name)
            except ValueError as e:
                print('Foreign key {} not deleted'.format(fk_name), e)
    ...
    def backwards(self, orm):
        for from_field, to_table, to_field in (
            ('type_id', 'ralph_scrooge_baseusage', 'id'),
            ('pricing_object_id', 'ralph_scrooge_pricingobject', 'id'),
            ('service_environment_id', 'ralph_scrooge_serviceenvironment', 'id'),
            ('warehouse_id', 'ralph_scrooge_warehouse', 'id'),
        ):
            sql = db.foreign_key_sql('ralph_scrooge_dailycost', from_field, to_table, to_field)
            db.execute(sql)
