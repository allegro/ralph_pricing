...
class Migration(DataMigration):
    ...
    def update_usage_id(self):
        for table, field in [
            # base usage
            ('ralph_scrooge_baseusage', 'id'),
            ('ralph_scrooge_usagetype', 'baseusage_ptr_id'),
            ('ralph_scrooge_extracosttype', 'baseusage_ptr_id'),
            ('ralph_scrooge_pricingservice', 'baseusage_ptr_id'),
            ('ralph_scrooge_dynamicextracosttype', 'baseusage_ptr_id'),
            ('ralph_scrooge_dailycost', 'type_id'),
            ('ralph_scrooge_team', 'baseusage_ptr_id'),

            # usage type
            ('ralph_scrooge_dynamicextracostdivision', 'usage_type_id'),
            ('ralph_scrooge_pricingservice_excluded_base_usage_types', 'usagetype_id'),
            ('ralph_scrooge_pricingservice_regular_usage_types', 'usagetype_id'),
            ('ralph_scrooge_serviceusagetypes', 'usage_type_id'),
            ('ralph_scrooge_usageprice', 'type_id'),
            ('ralph_scrooge_dailyusage', 'type_id'),

            # pricing service
            ('ralph_scrooge_pricingservice_excluded_services', 'pricingservice_id'),
            ('ralph_scrooge_pricingservice_regular_usage_types', 'pricingservice_id'),
            ('ralph_scrooge_historicalservice', 'pricing_service_id'),
            ('ralph_scrooge_service', 'pricing_service_id'),
            ('ralph_scrooge_serviceusagetypes', 'pricing_service_id'),

            # team
            ('ralph_scrooge_teamcost', 'team_id'),
            ('ralph_scrooge_teammanager', 'team_id'),

            # extra cost type
            ('ralph_scrooge_extracost', 'extra_cost_type_id'),

            # dynamic extra cost type
            ('ralph_scrooge_dynamicextracost', 'dynamic_extra_cost_type_id'),
            ('ralph_scrooge_dynamicextracostdivision', 'dynamic_extra_cost_type_id'),
        ]:
            print("Increasing IDs in {}".format(table))
            db.execute('UPDATE {0} SET {1} = {1} + 100'.format(
                table,
                field
            ))

    def bumped_auto_increment(self, value):
        """For sqlite3 bumped is not needed"""
        if engine == 'south.db.mysql':
            db.execute('ALTER TABLE ralph_scrooge_baseusage AUTO_INCREMENT = {}'.format(value))
