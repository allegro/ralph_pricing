...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for pricing_service in orm.PricingService.objects.all():
            if not pricing_service.use_universal_plugin:
                pricing_service.plugin_type = 2  # fixed price
                pricing_service.save()

    def backwards(self, orm):
        for pricing_service in orm.PricingService.objects.all():
            if pricing_service.plugin_type != 1:
                pricing_service.use_universal_plugin = False
                pricing_service.save()
