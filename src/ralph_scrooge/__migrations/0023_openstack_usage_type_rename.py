...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for ut in orm['ralph_scrooge.UsageType'].objects.filter(
            name__startswith='openstack.'
        ):
            ut.name = ut.name.replace('openstack', 'openstack.ceilometer')
            ut.symbol = ut.symbol.replace('openstack', 'openstack.ceilometer')
            ut.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for ut in orm['ralph_scrooge.UsageType'].objects.filter(
            name__startswith='openstack.ceilometer.'
        ):
            ut.name = ut.name.replace('openstack.ceilometer', 'openstack')
            ut.symbol = ut.symbol.replace('openstack.ceilometer', 'openstack')
            ut.save()
