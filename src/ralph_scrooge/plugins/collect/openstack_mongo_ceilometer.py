...
#    reducer = Code("""function( curr, result ) {result.total += curr.counter_volume;}""")
#    return db.meter.group(key={"project_id": 1, "counter_name": 1 },condition={"timestamp": { "$gt": yesterday, "$lt" : today }}, initial={"total" : 0}, reduce=reducer)
    cumulative_data = db.meter.aggregate([
    {"$match": {"timestamp": { "$gt": yesterday, "$lt" : today},"counter_type": "cumulative"}},
    {"$group": {"_id": { "project_id": "$project_id", "counter_name": "$counter_name"},"counter_max": {"$max":"$counter_volume"},"counter_min": {"$min":"$counter_volume"}}},
    {"$project": {"_id": { "project_id": 1, "counter_name":1}, "value": {"$subtract": ["$counter_max", "$counter_min"]}}},
    ])
    gauge_data = db.meter.aggregate([
    {"$match": {"timestamp": { "$gt": yesterday, "$lt" : today },"counter_type": "gauge"}},
    {"$group": {"_id": { "project_id": "$project_id", "counter_name": "$counter_name"},"total": {"$sum": "$counter_volume"}}},
    {"$project": {"_id": { "project_id": 1, "counter_name":1}, "value": {"$divide": ["$total", 144]}}},
    ])
    delta_data = db.meter.aggregate([
    {"$match": {"timestamp": { "$gt": yesterday, "$lt" : today },"counter_type": "delta"}},
    {"$group": {"_id": { "project_id": "$project_id", "counter_name": "$counter_name"},"value": {"$sum": "$counter_volume"}}},
    ])
    
    return cumulative_data, gauge_data, delta_data


def save_ceilometer_usages(cumulative_data, gauge_data, delta_data, date, warehouse):
    new = total = 0
...
def openstack_mongo_ceilometer(today, **kwargs):
    ...
    for site in settings.OPENSTACK_CEILOMETER:
        ...
        try:
            ...
            continue
        cumulative_data, gauge_data, delta_data = get_ceilometer_usages(today, site['CEILOMETER_CONNECTION'])
        site_new, site_total = save_ceilometer_usages(cumulative_data['result'], gauge_data['result'], delta_data['result'], today, warehouse)
